...
void USBsetSettingsDir()
{
	...
	{
#ifdef _WIN32
		IniPath = std::string(Path::Combine(GetSettingsFolder(), iniFile)); // default path, just in case
#else
		IniPath = std::string(Path::Combine(GetSettingsFolder().string(), iniFile)); // default path, just in case
#endif
		USBpathSet = true;
	}
}
