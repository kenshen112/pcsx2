...
static void _ApplySettings(const Pcsx2Config& src, Pcsx2Config& fixup)
{
	...
	if (fixup.EnableCheats)
		gameCheats.Printf(" [%d Cheats]", LoadPatchesFromDir((std::string)gameCRC, GetCheatsFolder(), "Cheats"));

	// wide screen patches
	if (fixup.EnableWideScreenPatches)
	{
		if (int numberLoadedWideScreenPatches = LoadPatchesFromDir((std::string)gameCRC, GetCheatsWsFolder(), "Widescreen hacks"))
		{
