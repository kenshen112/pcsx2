...
static void _ApplySettings(const Pcsx2Config& src, Pcsx2Config& fixup)
{
	...
	if (fixup.EnableCheats)
		gameCheats.Printf(L" [%d Cheats]", LoadPatchesFromDir(gameCRC, wxDirName(Path::ToWxString(GetCheatsFolder())), L"Cheats"));

	// wide screen patches
	if (fixup.EnableWideScreenPatches)
	{
		if (int numberLoadedWideScreenPatches = LoadPatchesFromDir(gameCRC, wxDirName(Path::ToWxString(GetCheatsWsFolder())), L"Widescreen hacks"))
		{
