...
static void _ApplySettings(const Pcsx2Config& src, Pcsx2Config& fixup)
{
	...
	if (fixup.EnableCheats)
		gameCheats.Printf(L" [%d Cheats]", LoadPatchesFromDir(gameCRC, wxDirName(Path::wxConvert(GetCheatsFolder())), L"Cheats"));

	// wide screen patches
	if (fixup.EnableWideScreenPatches)
	{
		if (int numberLoadedWideScreenPatches = LoadPatchesFromDir(gameCRC, wxDirName(Path::wxConvert(GetCheatsWsFolder())), L"Widescreen hacks"))
		{
		...
		else
		{
			...
			wxString cheats_ws_archive = Path::wxConvert(Path::Combine(PathDefs::GetProgramDataDir().string(), "cheats_ws.zip"));
