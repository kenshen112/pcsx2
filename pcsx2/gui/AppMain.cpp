...
public:
	PluginErrorEvent(BaseException* ex = NULL)
		: _parent(ex)
	{
	}
	PluginErrorEvent(const BaseException& ex)
		: _parent(ex)
	{
	}
...
public:
	PluginInitErrorEvent(BaseException* ex = NULL)
		: _parent(ex)
	{
	}
	PluginInitErrorEvent(const BaseException& ex)
		: _parent(ex)
	{
	}
...
void AppApplySettings(const GuiConfig* oldconf)
{
		...
		folderUtils.DoesExist(g_Conf->gui->Folders.Logs))
	{
			...
			folderUtils.CreateFolder(g_Conf->gui->Folders.Logs))
		{
			...
			/*if( (oldconf == NULL) || (oldconf->LanguageCode.CmpNoCase(g_Conf->gui->LanguageCode)) )
			{
				wxDoNotLogInThisScope please;
				i18n_SetLanguage( g_Conf->gui->LanguageId, g_Conf->gui->LanguageCode );
			}*/
...
{
	if (AppRpc_TryInvoke(&Pcsx2App::OpenGsPanel))
		return;
