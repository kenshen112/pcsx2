...
void Pcsx2App::OpenMainFrame()
{
...
#ifndef DISABLE_RECORDING
	VirtualPad* virtualPad0 = new VirtualPad(mainFrame, 0, g_Conf->gui->inputRecording);
	g_InputRecording.setVirtualPadPtr(virtualPad0, 0);
	m_id_VirtualPad[0] = virtualPad0->GetId();

	VirtualPad* virtualPad1 = new VirtualPad(mainFrame, 1, g_Conf->gui->inputRecording);
	...
	if (g_Conf->emulator->Debugger.ShowDebuggerOnStart)
		disassembly->Show();
...
void Pcsx2App::OpenProgramLog()
{
	...
	m_ptr_ProgramLog = new ConsoleLogFrame(GetMainFramePtr(), L"PCSX2 Program Log", g_Conf->gui->console);
...
void Pcsx2App::AllocateCoreStuffs()
{
	...
	if (!m_CpuProviders)
	{
		...
		if (m_CpuProviders->HadSomeFailures(g_Conf->emulator->Cpu.Recompiler))
		{
...
bool Pcsx2App::OnInit()
{
	...
	try
	{
		...
		if (Startup.ForceConsole)
			g_Conf->gui->console.Visible = true;
		OpenProgramLog();
		AllocateCoreStuffs();
		if (m_UseGUI)
			OpenMainFrame();
		...
		g_Conf->gui->CurrentIRX = "";

		if (Startup.SysAutoRun)
		{
			g_Conf->emulator->UseBOOT2Injection = !Startup.NoFastBoot;
			g_Conf->gui->CdvdSource = Startup.CdvdSource;
			if (Startup.CdvdSource == CDVD_SourceType::Iso)
				SysUpdateIsoSrcFile(Startup.IsoFile);
			sApp.SysExecute(Startup.CdvdSource);
			g_Conf->gui->CurrentGameArgs = Startup.GameLaunchArgs;
		}
		...
		else if (Startup.SysAutoRunIrx)
		{
			...
			g_Conf->gui->CurrentIRX = Startup.ElfFile;
