	...
	, m_thread(std::make_unique<GSThread>(this))
{
	...
	*this += debugger;
	...
	m_fs_watcher.Add(wxFileName(Path::ToWxString(g_Conf->Folders.Snapshots)));
...
void Dialogs::GSDumpDialog::GetDumpsList()
{
	...
	wxDir snaps(Path::ToWxString(g_Conf->Folders.Snapshots));
	wxString filename;
...
{
	wxString filename_preview = Path::ToWxString(g_Conf->Folders.Snapshots) + ("/" + evt.GetText()) + ".png";
	wxString filename = Path::ToWxString(g_Conf->Folders.Snapshots) + ("/" + evt.GetText()) + ".gs";
