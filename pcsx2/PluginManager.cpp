...
// ---------------------------------------------------------------------------------
StaticLibrary::StaticLibrary(PluginsEnum_t _pid)
	: pid(_pid)
{
}
	...
	: Filename(srcfile)
{
	...
	else
	{
		...
		if (!wxFile::Exists(Filename))
			throw Exception::PluginLoadError(pid).SetStreamName(srcfile).SetBothMsgs(pxL("The configured %s plugin file was not found"));

		if (!Lib->Load(Filename))
			throw Exception::PluginLoadError(pid).SetStreamName(Filename).SetBothMsgs(pxL("The configured %s plugin file is not a valid dynamic library"));
		...
		if (GetLibName == NULL || GetLibVersion2 == NULL)
			throw Exception::PluginLoadError(pid).SetStreamName(Filename).SetDiagMsg(L"%s plugin init failed: Method binding failure on GetLibName or GetLibVersion2.").SetUserMsg(_("The configured %s plugin is not a PCSX2 plugin, or is for an older unsupported version of PCSX2."));
	...
	if (testres != 0)
		throw Exception::PluginLoadError(pid).SetStreamName(Filename).SetDiagMsg(wxsFormat(L"Plugin Test failure, return code: %d", testres)).SetUserMsg(_("The plugin reports that your hardware or software/drivers are not supported."));
}
...
void SysCorePlugins::PluginStatus_t::BindCommon(PluginsEnum_t pid)
{
	...
	while (current->MethodName != NULL)
	{
		*target = (VoidMethod*)Lib->GetSymbol(current->GetMethodName(pid));
		...
		if (*target == NULL)
		{
			throw Exception::PluginLoadError(pid).SetStreamName(Filename).SetDiagMsg(wxsFormat(L"\nMethod binding failure on: %s\n", WX_STR(current->GetMethodName(pid)))).SetUserMsg(_("Configured plugin is not a PCSX2 plugin, or is for an older unsupported version of PCSX2."));
		}
...
void SysCorePlugins::PluginStatus_t::BindRequired(PluginsEnum_t pid)
{
	...
	while (current->MethodName != NULL)
	{
		*(current->Dest) = (VoidMethod*)Lib->GetSymbol(current->GetMethodName());
		...
		if (*(current->Dest) == NULL)
		{
			throw Exception::PluginLoadError(pid).SetStreamName(Filename).SetDiagMsg(wxsFormat(L"\n%s plugin init error; Method binding failed: %s\n", WX_STR(current->GetMethodName()))).SetUserMsg(_("Configured %s plugin is not a valid PCSX2 plugin, or is for an older unsupported version of PCSX2."));
		}
...
void SysCorePlugins::Load(const wxString (&folders)[PluginId_Count])
{
	...
	//Console.WriteLn(Color_StrongBlue, L"\nLoading plugins from %s...", (wxString)g_Conf->gui->Folders[FolderId_Plugins]);
...
void SysCorePlugins::Open()
{
	...
	if (!NeedsOpen())
		return; // Spam stopper:  returns before writing any logs. >_<
	...
	ForPlugins([&](const PluginInfo* pi) {
		...
		// gsopen -- Gregory
		if (pi->id == PluginId_GS)
			GetMTGS().WaitForOpen();
#else
		if (pi->id == PluginId_GS && !GSopen2)
			GetMTGS().WaitForOpen();
#endif
	});
...
void SysCorePlugins::ClosePlugin_Mcd()
{
	ScopedLock lock(m_mtx_PluginStatus);
	if (SysPlugins.Mcd)
		SysPlugins.Mcd->Base.EmuClose((PS2E_THISPTR)SysPlugins.Mcd);
}
...
{
	ScopedLock lock(m_mtx_PluginStatus);
	if (m_SettingsFolder.IsEmpty())
		return;

	ForPlugins([&](const PluginInfo* pi) {
		if (m_info[pi->id])
			m_info[pi->id]->CommonBindings.SetSettingsDir(m_SettingsFolder.utf8_str());
	});
}
...
void SysCorePlugins::SetSettingsFolder(const wxString& folder)
{
	...
	if (m_SettingsFolder == fixedfolder)
		return;
...
void SysCorePlugins::SendLogFolder()
{
	...
	ForPlugins([&](const PluginInfo* pi) {
		if (m_info[pi->id])
			m_info[pi->id]->CommonBindings.SetLogDir(m_LogFolder.utf8_str());
	});
}
