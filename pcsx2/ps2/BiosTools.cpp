...
void LoadBIOS()
{
	...
	try
	{
		...
		if( fs::is_directory(g_Conf->BaseFilenames.Bios))
			throw Exception::FileNotFound(Path::wxConvert(Bios))
		...
		wxString biosZone;
		wxFFile fp(Path::wxConvert(Bios), "rb");
		...
		pxInputStream memfp(Path::wxConvert(Bios), new wxMemoryInputStream(eeMem->ROM, sizeof(eeMem->ROM)));
...
{
	wxFileName Bios( Path::wxConvert(g_Conf->Folders.Bios) + filename );
