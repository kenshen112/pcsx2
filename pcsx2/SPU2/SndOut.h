...
struct Stereo51Out32Dpl
{
	...
	Stereo51Out32Dpl operator*(const int& factor) const
	{
	...
	Stereo51Out32Dpl& operator*=(const int& factor)
	{
		Left *= factor;
		Right *= factor;
		return *this;
	...
	Stereo51Out32Dpl operator+(const Stereo51Out32Dpl& right) const
	{
		return Stereo51Out32Dpl(
			Left + right.Left,
			Right + right.Right,
